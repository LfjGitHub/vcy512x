...
/******************************************************************************/
static uint8* setupFlagsAdData(uint8* ad_data, uint8* space, adv_discoverable_mode_t mode)
{
    uint16 flags = 0;
    ...
    if(flags)
    {
        ...
        ad_data = addHeaderToAdData(ad_data, space, 0x02, ble_ad_type_flags);
...
/******************************************************************************/
static uint8* addServiceUuidToAdData(uint8* ad_data, uint8* space, uint16 uuid)
{
    *ad_data = (uuid & 0xFF);
    *(ad_data + 1) = (uuid >> 8);
...
{ 
    ...
    {
        BLE_AD_INFO(("BAS "));
        ad_data = addServiceUuidToAdData(ad_data, space, GATT_SERVICE_UUID_BATTERY_SERVICE);
    }

    if (sinkGattLinkLossServiceEnabled() && (*space))
    {
        BLE_AD_INFO(("LLS "));
        ad_data = addServiceUuidToAdData(ad_data, space, GATT_SERVICE_UUID_LINK_LOSS);
    }

    if (sinkGattTxPowerServiceEnabled() && (*space))
    {
        BLE_AD_INFO(("TPS "));
        ad_data = addServiceUuidToAdData(ad_data, space, GATT_SERVICE_UUID_TX_POWER);
    }

    if (sinkGattImmAlertServiceEnabled() && (*space))
    {
        BLE_AD_INFO(("IAS "));
        ad_data = addServiceUuidToAdData(ad_data, space, GATT_SERVICE_UUID_IMMEDIATE_ALERT);
    }

    if (sinkGattHeartRateServiceEnabled() && (*space))
    {
        BLE_AD_INFO(("HRS "));
        ad_data = addServiceUuidToAdData(ad_data, space, GATT_SERVICE_UUID_HEART_RATE);
    }
    ...
    {
        BLE_AD_INFO(("RSCS"));
        ad_data = addServiceUuidToAdData(ad_data, space, GATT_SERVICE_UUID_RUNNING_SPEED_AND_CADENCE);
    }
...
/*
    ...
    {
        BLE_AD_INFO(("LOGGING"));
        ad_data = addServiceUuidToAdData(ad_data, space, UUID_LOGGING_SERVICE);
    }
*/
#endif
    ...
    {
        BLE_AD_INFO(("DIS "));
        ad_data = addServiceUuidToAdData(ad_data, space, GATT_SERVICE_UUID_DEVICE_INFORMATION);
    }
...
/******************************************************************************/
static uint8* setupServicesAdData(uint8* ad_data, uint8* space)
{
...
void bleSetupAdvertisingData(uint16 size_local_name, const uint8 *local_name, adv_discoverable_mode_t mode)
{
    ...
    if(ad_start)
    {
        ...
        ad_head = setupFlagsAdData(ad_head, &space, mode);
        

        name_space_reserved = reserveSpaceForLocalName(&space, size_local_name);
        
        ad_head = setupServicesAdData(ad_head, &space);
