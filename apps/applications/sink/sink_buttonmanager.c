...
static bool bmIsEdge(ButtonsTime_t pDuration);
static bool bmIsButtonInEvent(event_config_type_t *event_config, uint32 pButtonMask, ButtonsTime_t pDuration);
...
/****************************************************************************
NAME
 bmIsEventMaskInButtonMask

DESCRIPTION
 function to check if an event mask is in the button mask

RETURNS
    TRUE if event mask is in the button mask, FALSE otherwise
*/
static bool bmIsEventMaskInButtonMask(uint32 event_mask, uint32 button_mask)
{
...
/****************************************************************************
NAME
 bmIsEventMaskEqualButtonMask

DESCRIPTION
 function to check if an event mask is the same as a button mask

RETURNS
    TRUE if event mask is the same as the button mask, FALSE otherwise
*/
static bool bmIsEventMaskEqualButtonMask(uint32 event_mask, uint32 button_mask)
{
...
DESCRIPTION
 function to check if a duration is an edge

RETURNS
    TRUE if a duration is an edge, FALSE if duration is for a level
*/
static bool bmIsEdge(ButtonsTime_t pDuration)
{
    return ((pDuration == B_LOW_TO_HIGH) || (pDuration == B_HIGH_TO_LOW));
}
...
static bool bmIsButtonInEvent(event_config_type_t *event_config, uint32 pButtonMask, ButtonsTime_t pDuration)
{
    uint32 event_mask = bmGetButtonEventConfigPioMask(event_config);

    if ((ButtonsTime_t)event_config->type == pDuration)
    {
        if (bmIsEdge(pDuration))
        {
            return bmIsEventMaskInButtonMask(event_mask, pButtonMask);
        }
        else
        {
            return bmIsEventMaskEqualButtonMask(event_mask, pButtonMask);
        }
    }
    else
    {
