...
def get_obj_files():
    ...
    support_lib_output_path = os.path.join(KCS_OUTPUT_DIR, "support_lib")
    ...
    # folders are missing
    try:
        support_lib_names = get_immediate_subdirectories(support_lib_output_path)
    ...
    for support_lib in support_lib_names:
        # Exclude processing intermediate dyn object files
        o_files.extend([os.path.join(os.path.join(support_lib_output_path,support_lib),"debugbin","lib{}.a".format(support_lib))])
...
if __name__ == "__main__":
    ...
    for kdc_name in mod.KDCs.keys():
        ...
        build.build(rel_binfo, cap_list, kdc_name, LINKSCRIPT_PATH, MAKE_PATH, clean_build, MAKE_JOBS)
