 ...
 */
static unsigned get_rtp_header_timestamp(RTP_DECODE_OP_DATA *opx_data, unsigned header_size)
{

    tCbuffer *ip_buffer = opx_data->ip_buffer;
    unsigned int fixed_header_timestamp[RTP_FIXED_HEADER_TIMESTAMP_PART_LENGTH];
    uint32 rtp_timestamp;
    uint32 sample_diff;

    /* Get the sequence number */
    unpack_cbuff_to_array((int*)fixed_header_timestamp, ip_buffer, RTP_FIXED_HEADER_TIMESTAMP_PART_LENGTH);

    /* The RTP timestamp field is actually a sample count.
     * This sample count should be used to generate the time to play
     * timestamp . */
    rtp_timestamp = (((uint32)fixed_header_timestamp[0])<<24) + (fixed_header_timestamp[1]<<16) +
            (fixed_header_timestamp[2]<<8) + fixed_header_timestamp[3];

    /* Protected from the rtp timestamp overflow.*/
    sample_diff = (uint32) ((int32)rtp_timestamp - (int32)opx_data->last_rtp_timestamp);

    /* TODO make this a L4 log*/
    L2_DBG_MSG2("RTP decode APTX adaptive:\n    rtp_timestamp = %d\n    sample_diff=%d",
            rtp_timestamp, sample_diff);

    /* Save the RTP timestamp */
    opx_data->last_rtp_timestamp = rtp_timestamp;

    /* Discard the rest of the header. */
    cbuffer_advance_read_ptr_ex(ip_buffer, header_size -
            (RTP_FIXED_HEADER_FIRST_PART_LENGTH + RTP_FIXED_HEADER_TIMESTAMP_PART_LENGTH));
    return sample_diff;
}
    ...
    while ( (packet_size != 0) && (available_octets >= packet_size))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    header_calc_samples = get_rtp_header_timestamp(opx_data, header_size);
                }
                else
                {
                    header_calc_samples = 0;
                ...
                get_samples_in_packet(opx_data, &frame_data, header_calc_samples);
